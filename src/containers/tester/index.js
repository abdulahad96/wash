...
class Testscreen extends Component {
    ...
    getlatlng = (data) => {
        ...
        axios.get(`https://maps.googleapis.com/maps/api/place/details/json?key=AIzaSyCIGENLCfCwZwPaumiUQs21GfgMhgppa7s&placeid=${data.place_id}&language=en`).then((res) => {
            console.log(res)
            this.setState({ lat: res.data.result.geometry.location.lat, lng: res.data.result.geometry.location.lng, suggestedPlaces: [] })
    ...
    render() {
        ...
        return (
            <View>
                ...
                <View style={{ position: "absolute", width: Metrics.screenWidth, justifyContent: 'center', paddingHorizontal: 10, top: Metrics.ratio(50), backgroundColor: "#b4b4b4" }}>
                    ...
                    <View style={{ marginVertical: Metrics.ratio(10), backgroundColor: "white", paddingVertical: Metrics.ratio(-15), height: Metrics.ratio(35), borderRadius: Metrics.ratio(15) }}>
                        <TextInput style={{}} editable={true} value={this.state.text} placeholder="Enter Places" onChangeText={(e) => { this.getdata(e) }} />
                    </View>
                    <ScrollView>
